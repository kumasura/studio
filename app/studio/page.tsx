...
function StageNode({ data }: { data: NodeData }) {
  return (
    <div className="relative rounded-2xl shadow bg-white border border-zinc-200 p-3 w-64">
      {/* IN port */}
      <Handle type="target" position={Position.Left} id="in" style={{ width: 10, height: 10, background: '#111', borderRadius: 8 }} />
      ...
      {data.state && (
        <pre className="mt-2 text-[10px] bg-zinc-50 p-2 rounded-xl overflow-auto max-h-28">{JSON.stringify(data.state, null, 2)}</pre>
      )}

      {/* OUT port */}
      <Handle type="source" position={Position.Right} id="out" style={{ width: 10, height: 10, background: '#2563eb', borderRadius: 8 }} />
    </div>
  )
}
...
function RouterNode({ data }: { data: NodeData }) {
  return (
    <div className="relative rounded-2xl shadow bg-white border border-zinc-200 p-3 w-64">
      ...
      {/* OUT ports: yes/no */}
      <Handle type="source" position={Position.Bottom} id="yes" style={{ left: 20, width: 10, height: 10, background: '#16a34a', borderRadius: 8 }} />
      <Handle type="source" position={Position.Bottom} id="no" style={{ left: 60, width: 10, height: 10, background: '#dc2626', borderRadius: 8 }} />
    </div>
  )
}
...
function StudioInner() {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState<NodeData>([
    ...
    { id: 'tool1', type: 'stage', position: { x: 820, y: 20 }, data: { label: 'Tool', subtitle: 'Function call', tool: 'calc', params: { expression: '2+3*4' } } },
    { id: 'tool2', type: 'stage', position: { x: 820, y: 160 }, data: { label: 'Tool', subtitle: 'Function call', tool: 'weather', params: { city: 'Delhi' } } },
  ...
  const onDrop = (event: React.DragEvent) => {
    ...
    const newNode = { id, type: payload.type || 'stage', position, data: payload.data || payload }
    setNodes((nds) => nds.concat(newNode))
  }

  // Connect by dragging from blue (source) to black (target)
      ...
      <div className="grid grid-cols-12 gap-4 p-4">
        ...
        <div className="col-span-7 h-[75vh] rounded-2xl overflow-hidden border bg-white" ref={reactFlowWrapper}>
          ...
          >
            ...
            <Panel position="top-left">
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-xs p-2 bg-white/80 rounded-xl shadow">
                Drag from blue port → black port to connect. Add a Router to try multi-outputs.
              </motion.div>
            </Panel>
          </ReactFlow>
        </div>
