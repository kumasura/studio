...
// ======================================================================
// Optimization Flow Studio – Next.js (Vercel) MVP (v2)
// - React Flow graph of nodes for data → transform → modeling → solve
// - Multiple dataset streams via handles: ds:<name>  (default ds:main)
// - Topological dataset propagation with cycle detection
// - Constraint Builder supports data aggregates (sum/mean) and binding
//   aggregate coefficients to decision variables or as constants on LHS
// - Python transformer (Pyodide) can overwrite source column or write new
// - MILP solve via GLPK.js with BnB/BnC and MIP gap
// Drop this file into: app/opt/page.tsx
// ======================================================================
...
export type StreamName = string; // e.g. "main", "aux1"
...
interface DataBrowserData {
  dataset?: Dataset | null; // input (active stream resolved)
  activeStream?: StreamName; // which stream to view
}

interface TransformerData {
  code: string; // Python function body for def f(*xs)
  inputColumns: string[]; // wired columns
  outputColumn: string; // new column name
  overwrite?: boolean; // overwrite first input column
  status?: string;
  dataset?: Dataset | null; // active stream dataset
...
interface DecisionVarData {
  ...
  featureColumn?: string | null; // optional bound column
  dataset?: Dataset | null;
  activeStream?: StreamName;
}
...
interface ConstraintAggTerm {
  ...
  kind: AggKind; // sum | mean
  scale: number; // multiplier
  bindVar?: string | null; // if set, becomes coeff for that var; else constant term
}
...
interface ConstraintData {
  ...
  rhs: number;
  note?: string;
  inputColumns?: string[]; // connected columns (from graph)
  aggs?: ConstraintAggTerm[]; // editable aggregate terms
  dataset?: Dataset | null;
  activeStream?: StreamName;
}
...
// -------------- Global Graph Store (datasets per node & stream) --------------

interface GraphState {
  // map of nodeId -> streamName -> Dataset
...
function DataBrowserNode({ id, data }: NodeProps<DataBrowserData>) {
  ...
  return (
    <div className="rounded-2xl border bg-white shadow p-3 w-[460px]">
      ...
      {/* Dataset input + pass-through output */}
...
function TransformerNode({ id, data }: NodeProps<TransformerData>) {
  ...
  const outputCol = overwrite
    ? (inputCols[0] ?? data.outputColumn ?? "new_feature")
    : (data.outputColumn || "new_feature");
    ...
    <div className="rounded-2xl border bg-white shadow p-3 w-[460px]">
      ...
      <div className="mb-2 flex items-center gap-2">
        <input
          ...
          onChange={(e) => ((data as any).overwrite = e.target.checked)}
        />
        ...
        {outputCol && (
          <div className="relative border rounded px-2 py-1 bg-gray-50">
            ...
            <Handle
              ...
              id={`col:${overwrite && inputCols.length > 0 ? inputCols[0] : outputCol}`}
            />
          </div>
        )}
      </div>
  ...
  return (
    <div className="rounded-2xl border bg-white shadow p-3 w-[320px]">
      ...
      <div className="flex items-center gap-3 mb-2 text-[11px] text-gray-600">
        ...
        <div className="relative">
          <Handle type="target" position={Position.Left} id="col:feature" />
          feature
        </div>
      ...
      {data.featureColumn && (
        <div className="mb-2 text-[11px] text-gray-600">
          Bound column: <b>{data.featureColumn}</b>
        </div>
      )}
  ...
  return (
    <div className="rounded-2xl border bg-white shadow p-3 w-[460px]">
      ...
      <div className="mb-2">
        ...
        {(aggs.length === 0) && (
          <div className="text-[11px] text-gray-500 mt-1">(optional) Add sum/mean of columns; bind to a var or leave as constant.</div>
        )}
        {aggs.map((a, i) => (
          <div key={i} className="mt-2 p-2 border rounded">
            <div className="grid grid-cols-5 gap-2 items-center">
              ...
              >
                {[a.column, ...cols.filter((c) => c !== a.column)].map((c) => (
              ...
              <input
                className="border rounded px-2 py-1 text-xs"
                placeholder="bind to var (optional)"
                defaultValue={a.bindVar || ""}
                onBlur={(e) => {
...
function SolverNode({ id, data }: NodeProps<SolverData>) {
  ...
  const solve = async () => {
    ...
    try {
      ...
      varList.forEach((v, i) => (varIndex[v] = i + 1)); // 1-based
      ...
      // Build constraints including aggregates
      const constrRows = consNodes.map((cn) => {
        const coeffs = parseLinearExpr(cn.data.expr || ""); // var coeffs

        // Pull dataset attached to this constraint's active stream
        const nodeId = cn.id;
        ...
        let constTerm = 0; // aggregate constants to move to RHS
        ...
        // Adjust RHS by constant terms added on LHS: move to RHS
        const sense = cn.data.sense;
        let rhs = cn.data.rhs - constTerm;
...
// ---------------- Topological dataset propagation (with cycle detection) ----------------
function useDatasetPropagation(nodes: Node[], edges: Edge[]) {
  const setNodes = useNodesState(nodes)[1]; // only for setter typing
  const store = useGraphStore();

  useEffect(() => {
    // Build dataset edge graph on handles id starting with "ds:" or "dataset"
    ...
    const cyclic = order.length < all.size;
    if (cyclic) {
      console.warn("Dataset propagation: cycle detected; propagation may be incomplete.");
    }

    // Push datasets along edges in topo order
    const dsMap = store.datasets; // current datasets per node/stream
    const updates: Record<string, Record<StreamName, Dataset>> = {};

    order.forEach((src) => {
      const streams = dsMap[src] || {};
      const outs = outAdj[src] || [];
      outs.forEach(({ to, stream }) => {
        const ds = streams[stream];
        if (ds) {
          updates[to] = updates[to] || {};
          updates[to][stream] = ds; // last writer wins
        }
      });
    });
    ...
    // Apply updates into node.data so nodes can read their active datasets
    setNodes((nds) =>
      nds.map((n) => {
        if (updates[n.id]) {
  ...
  }, [nodes, edges, setNodes, useGraphStore]);
}
...
export default function Page() {
    ...
    () => [
      {
        ...
        type: "dataUpload",
        position: { x: 50, y: 80 },
        data: { mode: "file", dataset: null, stream: "main" } as DataUploadData,
      },
      {
        id: "db1",
        type: "dataBrowser",
        position: { x: 450, y: 60 },
        data: { dataset: null, activeStream: "main" } as DataBrowserData,
      },
      {
        id: "tr1",
        type: "transformer",
      ...
      },
      {
        ...
        data: { name: "x", varType: "integer", lower: 0, upper: 10, objectiveCoeff: 3, activeStream: "main" } as DecisionVarData,
      },
      {
        id: "y1",
        type: "decisionVar",
        position: { x: 700, y: 360 },
        data: { name: "y", varType: "integer", lower: 0, upper: 10, objectiveCoeff: 2, activeStream: "main" } as DecisionVarData,
      },
      {
        id: "c1",
        type: "constraint",
        position: { x: 980, y: 340 },
        data: { name: "cap", expr: "2*x + 3*y", sense: "<=", rhs: 18, activeStream: "main", aggs: [] } as ConstraintData,
      },
      {
        id: "s1",
        type: "solver",
  ...
  const initialEdges = useMemo<Edge[]>(
    () => [
      ...
      // Decision vars & constraints to solver
  ...
  // Expose nodes/edges for solver access
  useEffect(() => {
    (window as any).__rf = { getNodes: () => nodes, getEdges: () => edges };
  }, [nodes, edges]);

  // Dataset propagation hook
  useDatasetPropagation(nodes, edges);
  ...
  const onConnect = useCallback(
    (params: Edge | Connection) => {
      ...
      const srcCol = (params as any).sourceHandle?.startsWith("col:")
        ? (params as any).sourceHandle.split(":")[1]
        : null;
    ...
    <div className="h-screen w-screen grid grid-cols-[1fr_360px]">
      ...
      <div className="border-l bg-white/60">
        ...
        <div className="p-4 text-[11px] text-gray-500 border-t">
          <div className="font-medium mb-1">How to use (v2)</div>
          <ol className="list-decimal ml-4 space-y-1">
            <li>Upload a CSV in <b>Data Upload</b>, choose a stream (default <code>main</code>).</li>
            <li>See preview in <b>Data Browser</b>. Drag column chips to <b>Transformer</b>, <b>DecisionVar</b>, or <b>Constraint</b>.</li>
            <li>In <b>Transformer</b>, write <code>def f(*xs)</code>, choose overwrite or a new output column, then run.</li>
            <li>In <b>Constraint</b>, enter variable expression and add <b>Aggregate terms</b> (sum/mean). Bind an aggregate to a variable to make it a coefficient; leave empty to treat as constant (moved to RHS).</li>
            <li>Click <b>Solve</b> to run GLPK (BnB/BnC) with MIP gap.</li>
          </ol>
        </div>
      </div>
